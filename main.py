...
registered_clients: dict[
    str, ClientInfo
] = {}  # students that have joined a room and are being monitored
video_writers: dict[
    str, VideoWriter_fourcc
] = {}  # To hold VideoWriter objects for different clients/sessions
...
def handle_frame(
...
):  # TODO: get client id and send to that client only, and the host application
    if frame_data["student_id"] in registered_clients:
        ...
        detections = registered_clients[frame_data["student_id"]][
            "detection"
        ].detectFrameByFrame(img)
...
def writeFrameToFile(img: bytes, frame_data: dict[str, str]):
    ...
    filename = (
        frame_data["student_id"] + ".avi"
    )  # You can change the extension based on the codec you choose.

    # Initialize VideoWriter if it's not already set for this filename
    if filename not in video_writers:
        fourcc = cv2.VideoWriter_fourcc(
            *"XVID"
        )  # Define the codec. You can choose another one if needed.
